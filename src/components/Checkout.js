  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        minRows={4}
        onChange={(e) =>
          handleNewAddress((prev) => ({ ...prev, value: e.target.value }))
        }
...
const Checkout = () => {
   ...
   const addAddress = async (token, newAddress) => {
    try {
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Could not add address. Check that the backend is running, reachable and returns valid JSON.",
          { variant: "error" }
        );
      }
    }
   ...
   const deleteAddress = async (token, addressId) => {
    try {
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Could not delete address. Check that the backend is running, reachable and returns valid JSON.",
          { variant: "error" }
        );
      }
    }
   ...
   */
   const validateRequest = (items, addresses) => {
    ...
    if (Number(walletBalance) < totalCartValue) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        { variant: "warning" }
      );
   ...
   */
   const performCheckout = async (token, items, addresses) => {
    ...
    try {
      // Perform POST request to execute checkout
      await axios.post(
      ...
      // Update wallet balance locally after successful checkout
      const totalCartValue = getTotalCartValue(items);
      const currentBalance = localStorage.getItem("balance");
      const newBalance = currentBalance - totalCartValue;
  ...
  useEffect(() => {
    ...
    // Fetch addresses if a valid token exists
    if (token) {
      const addressData = await getAddresses(token);
      if (addressData) {
        setAddresses((prev) => ({ ...prev, all: addressData }));
      }
    } else {
            ...
            {addresses.all.length === 0 ? (
              ...
              addresses.all.map((addr) => (
                <Box
                  ...
                  alignItems="center"
                  border={`2px solid ${addresses.selected === addr._id ? "#00a278" : "#ccc"}`}
                  borderRadius="8px"
                  bgcolor={`${addresses.selected === addr._id ? "#00a27820" : "#FFFFFF"}`}
                  p="1rem"
                  my="0.5rem"
